...
Write-Host "Sempre que você salvar ou alterar um CSV ou index.html, ele será enviado automaticamente."

while ($true) {
    # Detecta CSVs novos/alterados ou index.html alterado nos últimos 3 segundos
    $files = Get-ChildItem -Path $folder -Filter "*.csv" | Where-Object {
        (Get-Date) - $_.LastWriteTime -lt (New-TimeSpan -Seconds 3)
    }
    $htmlChanged = Get-ChildItem -Path "$repo\index.html" | Where-Object {
        (Get-Date) - $_.LastWriteTime -lt (New-TimeSpan -Seconds 3)
    }
    ...
    if ($files.Count -gt 0 -or $htmlChanged.Count -gt 0) {
        # Vai para a pasta do repositório
        Set-Location $repo

        # Atualiza repositório com autostash
        try {
        ...
        # Atualiza os JSONs caso tenha CSVs novos
        if ($files.Count -gt 0) {
            python gerar_ultimo.py
        }

        # Adiciona arquivos para commit
        foreach ($file in $files) { git add "dados_para_painel/$($file.Name)" }
        git add "historico/ultimo.json" "historico/historico.json"
        if ($htmlChanged.Count -gt 0) { git add "index.html" }

        # Comita se houver mudanças
        $hasChanges = git diff --cached --quiet
        if (-not $hasChanges) {
            git commit -m "Atualização automática: $($files | ForEach-Object { $_.Name }) + JSONs/index.html"
        }
